-- 029 — the refusal must come before the shredder
--
-- Migration 027 taught anonymise_account to refuse while work is still
-- uninvoiced. It refuses in the right place and for the right reason, and it is
-- still too late: lib/anonymise.ts deletes every document FILE from the storage
-- bucket BEFORE it calls the function.
--
-- So the refused erasure is not a no-op. The transaction rolls back, the
-- `documents` rows survive with status 'approved', the screen says "there are
-- still uninvoiced assignments, approve those hours first, then this account can
-- be anonymised" — come back later, nothing changed — and meanwhile the VOG, the
-- diploma and the insurance certificate are gone from the bucket. Every facility
-- holding a live assignment sees an approved document whose signed URL 404s, and
-- the owner cannot re-upload, because deleteDocument refuses approved rows.
--
-- The population that hits this refusal is exactly the population with live
-- assignments, and therefore exactly the population with documents.
--
-- That is the orphaned-file outcome anonymiseAccount's own comment says must
-- never happen, produced not by a failed remove but by the ORDER of two calls.
--
-- The fix is not to duplicate the count in TypeScript. A rule written twice
-- drifts — that is class (f), and this codebase has it in every audit. Instead
-- the rule moves into one function that BOTH callers read: anonymise_account for
-- its own refusal, and lib/anonymise.ts to ask "may I?" before it touches
...
/*
 * Why is this checked twice rather than moved out of the function entirely?
 *
 * Because the caller's check is advisory and the function's is authoritative.
 * Between the two, hours can be approved, an invoice can be raised, a shift can
 * be accepted. Only the check inside the transaction can actually hold. The
 * caller's job is narrower: do not start shredding something the transaction is
...
-- ---------------------------------------------------------------------------
-- anonymise_account, now reading that function instead of repeating it.
-- ---------------------------------------------------------------------------
